...
def print_segments(segments, printing_all):
    print(f'{"START ADDRESS":16s} {"PSS":>9s} {"RSS":>9s} {"DIRTY":>9s} {"VSIZE":>10s} {"PERMS":6s} {"TYPE":10s} FILE/THREAD')
    for seg in segments:
        if seg.type == '[stack]':
            ...
            detail = ''
        print(f'{seg.start:016x} {seg.pss:9d} {seg.rss:9d} {seg.dirty:9d} {seg.vsize:10d} {seg.perms:6s} {seg.type:10s} {detail}')
